...
import json
import yaml
from stream_query import query_rag_streaming 
...
def stream_query():
    ...
    def generate():
        try:
            ...
            for chunk in query_rag_streaming(query_text, buffer, session_name):
                yield chunk
        except Exception as e:
            yield f"Error: {str(e)}"
...
def save_chat_history():
    ...
    try:
        memory = create_or_get_buffer(session_name)
...
#     response_text = query_rag(message, buffer, session_name)
